...
import { fetchWebMediaUpstream } from '../_shared/webMediaTransport.ts';
...
serve(async request => {
  ...
  try {
    ...
    const upstream = await fetchWebMediaUpstream(payload.url, request);
    ...
    if (request.method === 'GET' && isManifest(upstream, payload.url)) {
      ...
      if (!manifest.trim().startsWith('#EXTM3U')) throw new Error('WEB_MEDIA_MANIFEST_INVALID');
    ...
    console.error('web-player-media error', {
      code: code.startsWith('WEB_MEDIA_') ? code : 'WEB_MEDIA_UPSTREAM_ERROR',
    });
