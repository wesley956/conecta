...
async function recordAttempt(supabase: ReturnType<typeof serviceClient>, codeHash: string, ipHash: string, success: boolean) {
  await supabase.from('web_player_login_attempts').insert({
    code_hash: codeHash,
    ip_hash: ipHash,
    success,
  });
...
async function throttled(supabase: ReturnType<typeof serviceClient>, codeHash: string, ipHash: string) {
  ...
  const [{ count: codeCount }, { count: ipCount }] = await Promise.all([
    supabase.from('web_player_login_attempts')
...
async function login(request: Request, body: Record<string, unknown>) {
  ...
  if (eligible && device) {
    ...
    // Mantém custo criptográfico semelhante para reduzir side-channel de enumeração.
    const dummySalt = 'MDEyMzQ1Njc4OWFiY2RlZg';
    await deriveWebPin(pin, dummySalt, 210000).catch(() => undefined);
  }
  ...
  if (sessionError || !sessionId) {
    ...
    if (message.includes('WEB_SESSION_LIMIT_REACHED')) {
      return webJson(request, {
        ok: false,
        code: 'WEB_SESSION_LIMIT_REACHED',
        message: 'O limite de navegadores ativos para este acesso foi atingido.',
      }, 409);
    }
...
async function refresh(request: Request, body: Record<string, unknown>) {
  ...
  if (
    !device || device.status !== 'active' || !device.web_access_enabled ||
    idleAt <= now || absoluteAt <= now || subscriptionAt <= now
  ) {
  ...
  const [nextAccessHash, nextRefreshHash] = await Promise.all([
    sha256Hex(nextAccessToken),
