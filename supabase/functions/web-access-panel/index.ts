...
function allowedOrigins() {
  ...
  return new Set([...configured, 'http://localhost:3000', 'http://localhost:4173', 'http://127.0.0.1:3000']);
}
...
async function status(request: Request, supabase: ReturnType<typeof serviceClient>, principal: Awaited<ReturnType<typeof requirePanelPrincipal>>, deviceId: string) {
  ...
  return json(request, {
    ok: true,
    device: {
      id: device.id, deviceCode: device.device_code, clientName: device.client_name || null, status: device.status,
      expiresAt: device.subscription_expires_at,
      webAccessEnabled: device.status === 'active' && Boolean(device.web_access_enabled),
      pinConfigured: Boolean(device.web_pin_hash), pinUpdatedAt: device.web_pin_updated_at || null,
      sessionLimit: Number(device.web_session_limit || 2), activeSessions: activeSessions.length,
    },
    sessions: (sessions || []).map(row => ({
      id: row.id, idShort: String(row.id).slice(0, 8), createdAt: row.created_at, lastUsedAt: row.last_used_at,
      idleExpiresAt: row.idle_expires_at, absoluteExpiresAt: row.absolute_expires_at, revokedAt: row.revoked_at,
      revokeReason: row.revoke_reason || null, browserFamily: row.user_agent_family || 'Browser',
...
serve(async request => {
  ...
  try {
    ...
    const device = await authorizedDevice(supabase, principal, deviceId);
    await enforceWebRateLimit(supabase, 'panel', `actor:${principal.userId}:device:${deviceId}`);
    ...
    if (code === 'WEB_RATE_LIMITED') return json(request, { ok: false, code }, 429);
