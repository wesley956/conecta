...
export async function requirePanelPrincipal(
  ...
  if (roleError) {
    throw new PanelAuthError(`Falha ao validar papel do painel: ${roleError.message}`, 500);
  }
  ...
  if (role === 'seller') {
    ...
    if (sellerError) {
      throw new PanelAuthError(`Falha ao validar vendedor: ${sellerError.message}`, 500);
    }
