-- Atualiza as trocas de listas para usar a qualificação comercial como fonte
-- única de verdade. O caminho de aparelhos existe em todas as instalações.
-- O caminho de assinaturas é criado somente quando esse domínio estiver
-- presente, evitando que instalações legadas recebam tabelas parciais.

do $guard$
begin
  if to_regclass('public.panel_subscriptions') is not null
     and to_regclass('public.panel_subscription_playlists') is not null
     and to_regclass('public.panel_subscription_devices') is not null
     and to_regclass('public.panel_subscription_operations') is not null
     and to_regclass('public.panel_playlist_revisions') is not null then
    execute $ddl$
      ...
      begin
        ...
        v_fingerprint := concat_ws('|', 'replace-playlist-v2', p_subscription_id, p_priority, p_candidate_playlist_id);
        ...
        perform public.assert_playlist_commercially_usable(p_candidate_playlist_id, 'A nova lista');
        ...
        insert into public.panel_subscription_playlists (
          subscription_id, playlist_id, priority, active, assigned_at
        ) values (
          p_subscription_id, p_candidate_playlist_id, p_priority, true, now()
        );
        ...
        insert into public.panel_playlist_revisions (
          subscription_id, priority, previous_playlist_id, new_playlist_id,
          reason, performed_by, performed_by_user_id
        ) values (
          p_subscription_id, p_priority, v_old_playlist_id, p_candidate_playlist_id,
          v_reason, p_performed_by, p_performed_by_user_id
        );
        ...
        insert into public.panel_audit_logs (
          ...
          p_subscription_id,
          format(
            'Lista %s da assinatura substituída após homologação comercial.',
          ...
          jsonb_build_object(
            ...
            'qualificationStatus', v_candidate.playlist_qualification_status
          )
        );
...
begin
  ...
  v_fingerprint := concat_ws('|', 'replace-device-playlist-v2', p_device_id, p_priority, p_candidate_playlist_id);
  ...
  insert into public.panel_device_playlists (
    ...
    p_device_id, p_candidate_playlist_id, p_priority, true, 0,
  ...
  insert into public.panel_device_playlist_revisions (
    ...
    p_device_id, v_device.seller_id, p_priority, v_old_playlist_id,
    p_candidate_playlist_id, v_reason, p_performed_by, p_performed_by_user_id
  );
  ...
  insert into public.panel_audit_logs (
    ...
    p_device_id,
    format(
      'Lista %s do aparelho substituída após homologação comercial.',
      case when p_priority = 1 then 'principal' else 'reserva' end
    ),
    jsonb_build_object(
      'priority', p_priority,
