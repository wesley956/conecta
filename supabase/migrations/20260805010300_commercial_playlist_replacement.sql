...
-- única de verdade. Preserva idempotência, auditoria, limites e histórico.
...
begin
  ...
  if v_old_playlist_id is not null
     and not exists (
       select 1 from public.panel_subscription_playlists subscription_playlist
       where subscription_playlist.playlist_id = v_old_playlist_id
         and subscription_playlist.active is true
     )
     and not exists (
       select 1 from public.panel_device_playlists device_playlist
...
begin
  ...
  if v_old_playlist_id is not null
     ...
     and not exists (
       select 1 from public.panel_devices device_record
