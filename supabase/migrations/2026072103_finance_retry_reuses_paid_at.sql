...
begin
  if p_finance_amount_cents is not null
     ...
     and lower(trim(coalesce(p_finance_status, 'pending'))) = 'paid' then
    ...
    v_stable_paid_at := coalesce(v_stable_paid_at, p_paid_at, now());
  else
...
comment on function public.apply_device_subscription_with_finance(
  ...
  'Entrada service_role da transação comercial-financeira. Em retries, prioriza o paid_at já persistido antes de qualquer horário reenviado pela API.';
