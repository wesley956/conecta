...
begin
  ...
  if p_enforce_seller_ownership
     and v_device.seller_id is not null
     and v_device.seller_id <> p_seller_id then
  ...
  -- O gatilho panel_credit_ledger_consume_lots é a única fonte de consumo
  -- dos lotes e aplica FIFO por expires_at e created_at.
  return query
  select
    true,
    v_ledger_id,
...
comment on function public.apply_device_subscription_transaction(
  ...
  'Ativa ou renova o aparelho e registra uma única saída no extrato. O gatilho do extrato consome os lotes em FIFO, evitando desconto duplicado.';
