...
function trackName(label: string, language: string | undefined, fallback: string) {
  const clean = String(label || "").trim();
  if (clean) return clean.slice(0, 120);
  const normalized = String(language || "").trim().replace(/_/g, "-");
  return normalized ? normalized.toUpperCase().slice(0, 35) : fallback;
}
...
export function PlayerScreen({
  ...
  return <main className="player-screen">
    ...
    {trackPanel && <aside className="track-panel"><header><div><p className="eyebrow">REPRODUÇÃO</p><h2>Áudio e legendas</h2><small>Foco vermelho • opção ativa marcada</small></div><button data-tv-focusable="true" onClick={() => setTrackPanel(false)}>×</button></header><section><h3>Faixa de áudio</h3>{snapshot.audioTracks.length === 0 && <p>Nenhuma faixa alternativa foi informada pelo conteúdo.</p>}{snapshot.audioTracks.map((track, index) => <button key={`audio:${track.index}`} data-tv-focusable="true" data-autofocus={snapshot.selectedAudioTrack === track.index || (snapshot.selectedAudioTrack == null && index === 0) ? "true" : undefined} className={snapshot.selectedAudioTrack === track.index ? "selected" : ""} onClick={() => { adapter.current?.selectTrack("audio", track.index); setTrackPanel(false); }}><span>♪</span><strong>{trackName(track.label, track.language, `Áudio ${index + 1}`)}</strong><small>{snapshot.selectedAudioTrack === track.index ? "ATIVA" : track.language?.toUpperCase() || "ÁUDIO"}</small></button>)}</section><section><h3>Legendas</h3><button data-tv-focusable="true" data-autofocus={snapshot.selectedTextTrack == null ? "true" : undefined} className={snapshot.selectedTextTrack == null ? "selected" : ""} onClick={() => { adapter.current?.selectTrack("text", null); setTrackPanel(false); }}><span>CC</span><strong>Desativadas</strong><small>{snapshot.selectedTextTrack == null ? "ATIVA" : "SEM LEGENDA"}</small></button>{snapshot.textTracks.map((track, index) => <button key={`text:${track.index}`} data-tv-focusable="true" data-autofocus={snapshot.selectedTextTrack === track.index ? "true" : undefined} className={snapshot.selectedTextTrack === track.index ? "selected" : ""} onClick={() => { adapter.current?.selectTrack("text", track.index); setTrackPanel(false); }}><span>CC</span><strong>{trackName(track.label, track.language, `Legenda ${index + 1}`)}</strong><small>{snapshot.selectedTextTrack === track.index ? "ATIVA" : track.language?.toUpperCase() || "LEGENDA"}</small></button>)}</section></aside>}
  </main>;
}
