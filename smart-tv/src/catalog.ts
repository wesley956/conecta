...
async function loadCatalog(session: DeviceSession, signal: AbortSignal): Promise<Catalog> {
  ...
  if (!urls?.channelsUrl || !urls.moviesUrl || !urls.seriesUrl) {
    throw new Error(session.cacheError || "O catálogo seguro ainda não está pronto.");
  }
  const [channels, movies, series] = await Promise.all([
    download(urls.channelsUrl, signal),
    download(urls.moviesUrl, signal),
    download(urls.seriesUrl, signal)
  ]);
  return {
    channels: items<Channel>(channels, "channels"),
    movies: items<Movie>(movies, "movies"),
    series: items<Series>(series, "series")
  };
}
...
export function useCatalog(session: DeviceSession, renewConfiguration: () => Promise<DeviceSession>) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        // A renovação gera links assinados novos; links temporários nunca são persistidos.
