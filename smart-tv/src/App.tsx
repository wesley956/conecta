  ...
  return (
    <main className="activation-shell">
      <section className="activation-panel">
        ...
        {session.deviceCode && (
          <div className="activation-code"><small>CÓDIGO DO APARELHO</small><strong>{session.deviceCode}</strong><span>{pending ? "Aguardando liberação automática" : "Identidade do aparelho"}</span></div>
        )}
        <div className="activation-actions">
          <FocusableButton data-autofocus="true" className="primary" disabled={session.refreshing || session.status === "loading"} onClick={onRefresh}>
...
function Poster({ image, name }: { image?: string; name: string }) {
  return image
...
export function App() {
  ...
  }, [selected, session.status, catalog.status]);

  const counts = useMemo(() => ({
    channels: catalog.data.channels.length,
    movies: catalog.data.movies.length,
    series: catalog.data.series.length
  }), [catalog.data]);

  const cards = selected === "TV ao vivo"
    ? catalog.data.channels.slice(0, 60).map(item => ({ id: item.id, name: item.name, image: item.logo, meta: item.groupTitle || "TV ao vivo" }))
    : selected === "Filmes"
      ? catalog.data.movies.slice(0, 60).map(item => ({ id: item.id, name: item.name, image: item.cover, meta: [item.category, item.year || ""].filter(Boolean).join(" • ") }))
      : selected === "Séries"
        ? catalog.data.series.slice(0, 60).map(item => ({ id: item.id, name: item.name, image: item.cover, meta: item.category || "Séries" }))
        : [];
    ...
    <main className="shell">
      <aside className="rail">
        ...
        <nav>{destinations.map(item => (
          <FocusableButton key={item.label} className={`nav-item ${selected === item.label ? "selected" : ""}`} onClick={() => setSelected(item.label)}>
      ...
      <section className="content">
        <header>
          <div><p className="eyebrow">RONECAPLAYTV</p><h1>{selected}</h1></div>
          <div className="header-actions">
            <div className="status"><i /> {session.clientName || "Aparelho ativo"} <span>•</span> <b>Ativo</b></div>
          </div>
        </header>
          ...
          <section className="hero">
            ...
            <div className="hero-copy">
              ...
              <div className="hero-actions">
                <FocusableButton data-autofocus="true" className="primary" onClick={() => setSelected("Filmes")}>Explorar filmes</FocusableButton>
                <FocusableButton className="secondary" onClick={() => setSelected("TV ao vivo")}>TV ao vivo</FocusableButton>
              </div>
            </div>
            <div className="hero-art"><div className="orb" /><span>R</span></div>
          </section>
          <section className="explore-section">
            <div className="section-heading"><div><h3>Explorar</h3><p>Conteúdo real da sua lista</p></div><div className="cut"><i /><i /></div></div>
            <div className="cards">
              {[
                ["TV ao vivo", `${counts.channels.toLocaleString("pt-BR")} canais`, "gold"],
                ["Filmes", `${counts.movies.toLocaleString("pt-BR")} títulos`, "red"],
                ["Séries", `${counts.series.toLocaleString("pt-BR")} séries`, "gold"]
              ].map(([label, count, tone]) => (
                <FocusableButton key={label} className={`card ${tone}`} onClick={() => setSelected(label)}>
                  <span className="card-icon">◆</span><span><strong>{label}</strong><small>{count}</small></span><b>›</b>
                </FocusableButton>
              ))}
            </div>
          </section>
        ...
        {catalog.status === "ready" && cards.length > 0 && (
          <section className="catalog-grid">
            {cards.map((item, index) => (
              <FocusableButton key={item.id} data-autofocus={index === 0 ? "true" : undefined} className="media-card">
                <span className="poster"><Poster image={item.image} name={item.name} /></span>
        ...
        {catalog.status === "ready" && !["Início", "TV ao vivo", "Filmes", "Séries"].includes(selected) && (
          <section className="state-panel"><h2>{selected}</h2><p>Esta área será conectada nos próximos marcos.</p></section>
        )}
        {catalog.status === "ready" && ["TV ao vivo", "Filmes", "Séries"].includes(selected) && cards.length === 0 && (
          <section className="state-panel"><h2>Nenhum conteúdo encontrado</h2><p>Esta categoria está vazia na lista vinculada.</p></section>
        )}
      </section>
    </main>
  );
