...
if (source.wizard.includes('T23:59:59.999Z')) {
  throw new Error('Wizard não pode reconstruir validade como UTC no fim do dia.');
}
if (/\balert\s*\(/.test(source.wizard)) {
  throw new Error('Validação do wizard deve aparecer dentro da etapa, não em alert().');
}
...
console.log('✅ Lote 4: ativação guiada, cadastro inline, saldo, renovação recente, fuso e responsividade validados.');
