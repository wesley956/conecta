...
`;
  ...
  assert(adultChannel.groupTitle === 'Adultos 18+', 'Categoria adulta deve preservar o grupo original.');
  assert(adultChannel.group === 'adultos-18', `Slug adulto inesperado: ${adultChannel.group}`);
  ...
  const dirtyInput = '\uFEFF#extm3u\r' +
    '#extinf : -1 tvg-id="lower-1" tvg-name="Canal Baixo" tvg-logo="https://cdn.exemplo/baixo.png" group-title="Teste",Canal Baixo\r' +
  ...
  const invalidAndMultiline = `#EXTM3U
#EXTINF:-1 tvg-name="Sem URL" group-title="Falhas",Sem URL
não-é-uma-url
#EXTINF:-1 tvg-name="URL depois de comentários" group-title="Teste",URL depois de comentários
#EXTVLCOPT:http-user-agent=VLC
#KODIPROP:inputstream=inputstream.adaptive
https://servidor.example/stream/live.m3u8?token=a%20b&x=1
`;
  const invalidResult = parseM3U(invalidAndMultiline, 'invalid-list');
  assert(invalidResult.channels.length === 1, 'Parser deveria pular entrada inválida e encontrar URL posterior.');
  assert(invalidResult.skipped === 1, `Esperava uma entrada ignorada, recebeu ${invalidResult.skipped}.`);
  assert(
    invalidResult.channels[0].url.includes('token=a%20b&x=1'),
    'URL com query string e caracteres escapados deveria ser preservada.',
  );
  ...
  assert(largeResult.movies.length === 0, 'Lista grande de canais não deveria criar filmes.');
  assert(largeResult.series.length === 0, 'Lista grande de canais não deveria criar séries.');

  console.log('✅ Parser M3U validado: atributos, normalização, multiline, erros e 10 mil entradas.');
} finally {
