...
const required = {
  financeUi: [
    '__ronecaFinanceReadModelInstalled',
    'Minhas vendas',
    ...
    'Este módulo não ativa, renova nem troca listas',
    'window.RonecaFinanceModule',
  ],
  financeCss: ['.finance-metrics', '.finance-status.overdue', '.seller-finance-record'],
  creditUi: [
    'Financeiro da empresa', 'Venda de créditos', 'Meus créditos',
  ...
  creditCss: ['[data-tab="finance"]', '#section-finance', '.credit-package-metrics', '.credit-package-option.featured', '.credit-seller-orders'],
  financeEdge: [
  ...
  financeMigration: [
    'create table if not exists public.panel_financial_records', 'force row level security',
    'apply_device_subscription_with_finance', 'panel_financial_records_idempotency_idx',
  ],
  packageMigration: [
...
for (const [name, snippets] of Object.entries(required)) {
  for (const snippet of snippets) {
    if (!source[name].includes(snippet)) throw new Error(`Proteção financeira ausente em ${files[name]}: ${snippet}`);
  }
}

for (const forbidden of [
  'activatePending', 'renewDevice', 'sellerUxActivateDevice', 'sellerUxRenewDevice',
  'activateDeviceWithFinance', 'renewDeviceWithFinance', '__financeAdminActivationInstalled', '__financeAdminRenewInstalled',
]) {
