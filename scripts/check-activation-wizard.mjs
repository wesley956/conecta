...
const atomic=fs.readFileSync('supabase/migrations/20260806051000_atomic_provisional_activation.sql','utf8');
...
assert.ok(edge.includes("action === 'renew'"));
assert.ok(edge.includes("action === 'changePlaylists'"));
assert.ok(edge.includes('activate_device_provisional_transaction'));
...
assert.ok(atomic.includes('apply_device_subscription_transaction'));
assert.ok(atomic.includes('change_device_playlists_transaction'));
