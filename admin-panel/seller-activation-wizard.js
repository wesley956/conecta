(() => {
  ...
  function qualificationNotice(id) {
    ...
    return '<div class="aw-notice warn"><strong>Ativação provisória</strong><span>Em Android, o backend pode liberar a lista para confirmação automática no aparelho.</span></div>';
  }
  ...
  function ensureModal() {
    ...
    root.innerHTML = `
      <div class="activation-wizard-card" role="dialog" aria-modal="true" aria-labelledby="activationWizardTitle">
        <header class="activation-wizard-head">
          <div><span class="activation-wizard-kicker">Fluxo comercial</span><h2 id="activationWizardTitle"></h2><p id="activationWizardSubtitle"></p></div>
          <button type="button" class="btn" data-aw-action="close" aria-label="Fechar">×</button>
        </header>
    ...
    root.addEventListener('click', event => {
      ...
      else if (action === 'new-playlist') newPlaylist(event.target.closest('[data-aw-field-target]')?.dataset.awFieldTarget);
      else if (action === 'step') go(Number(event.target.closest('[data-aw-step]')?.dataset.awStep || 1));
    });
  ...
  function syncInputs() {
    document.querySelectorAll('#activationWizard [data-aw-field]').forEach(input => {
      ...
      state.draft[name] = input.type === 'checkbox' ? input.checked : input.value;
    });
  }
  ...
  function render() {
    ...
    $('activationWizardTitle').textContent = state.mode === 'activation'
      ? 'Ativar aparelho' : state.mode === 'renewal' ? 'Renovar aparelho' : 'Alterar listas';
    $('activationWizardSubtitle').textContent = state.mode === 'activation'
      ? `${state.device?.deviceCode || ''} · cliente, plano e listas em uma única confirmação`
      : state.mode === 'renewal'
    ...
    $('activationWizardSteps').innerHTML = labels.map((label, index) => {
      ...
      return `<button type="button" class="aw-step ${number === state.step ? 'active' : ''} ${number < state.step ? 'done' : ''}" data-aw-action="step" data-aw-step="${number}"><span>${number < state.step ? '✓' : number}</span>${esc(label)}</button>`;
    }).join('');
    $('activationWizardBody').innerHTML = renderStep();
    $('activationWizardFooter').innerHTML = `
      ${state.step > 1 ? '<button type="button" class="btn" data-aw-action="back">Voltar</button>' : '<span></span>'}
      <div class="aw-footer-actions">
        <button type="button" class="btn" data-aw-action="close">Cancelar</button>
        ${state.step < labels.length
          ? '<button type="button" class="btn primary" data-aw-action="next">Continuar</button>'
          : `<button type="button" class="btn primary" data-aw-action="submit">${state.mode === 'activation' ? 'Ativar aparelho' : state.mode === 'renewal' ? 'Renovar aparelho' : 'Salvar novas listas'}</button>`}
      </div>`;
  }
  ...
  async function openActivation(deviceCode) {
    ...
    if (!code) throw new Error('Informe o código do aparelho.');
    ...
    await refreshData();
    state.mode = 'activation'; state.step = 1; state.device = lookup.device;
    state.draft = { customerName: lookup.device.customerName || '', customerWhatsapp: lookup.device.customerWhatsapp || '' };
    state.busy = false; clearAttempt();
  ...
  function close() {
    $('activationWizard')?.classList.remove('open');
    state.busy = false;
    state.attempt = null;
  }
  ...
  async function newPlaylist(field) {
    ...
    const observer = new MutationObserver(async () => {
      ...
      try {
        ...
        if (created) state.draft[field] = created.id;
        clearAttempt();
        render();
  ...
  function toggleBackup(enabled) {
    syncInputs();
    state.draft.useBackup = Boolean(enabled);
    if (!enabled) state.draft.backupPlaylistId = '';
    clearAttempt();
    render();
  }
  ...
  function go(step) {
    try {
      if (step > state.step) validateStep(); else syncInputs();
      state.step = Math.max(1, Math.min(step, steps().length));
      render();
    } catch (error) { alert(error.message); }
  }
  ...
  async function submit() {
    try {
      ...
      if (status) status.textContent = 'Processando uma única transação…';
      ...
      if (state.mode === 'activation') {
        ...
        payload = {
          ...
          expiresAt: d.expiresAt ? new Date(`${d.expiresAt}T23:59:59.999Z`).toISOString() : null,
        };
        ...
        payload = {
          ...
          playlistId: d.playlistId,
          backupPlaylistId: backup,
          reason: 'Alteração pelo fluxo comercial do vendedor',
        };
      }
      ...
      await window.RonecaSellerPortal?.refresh?.();
      setTimeout(close, 350);
    } catch (error) {
      const status = $('awSubmitStatus');
      if (status) status.textContent = error.message || 'Não foi possível concluir.';
      else alert(error.message || 'Não foi possível concluir.');
    } finally {
      state.busy = false;
    }
  }

  window.RonecaSellerDeviceFlowUI = Object.freeze({
    openActivation,
    openRenewal,
    openChange,
    close,
