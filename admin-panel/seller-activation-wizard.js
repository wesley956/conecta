(() => {
  ...
  function projectedExpiry(plan, customDate = '', renewal = false) {
    ...
    } catch { /* backend fará a validação definitiva */ }
  ...
  function planOptions(selected = '') {
    return '<option value="">Escolha um plano</option>' + (state.data?.plans || []).map(item => `<option value="${esc(item.id)}" ${item.id === selected ? 'selected' : ''}>${esc(item.name)} · ${Number(item.durationDays || 30)} dias · ${Number(item.creditCost || 1)} crédito(s)</option>`).join('');
  }

  function balanceSummary(plan) {
    const before = Number(state.data?.seller?.creditBalance || 0);
  ...
  function ensureModal() {
    ...
    root.addEventListener('click', event => {
      ...
      const actionNode = event.target.closest('[data-aw-action]');
      const action = actionNode?.dataset.awAction;
      if (!action) return;
      if (action === 'close') close();
      else if (action === 'back') back();
      else if (action === 'next') next();
      else if (action === 'submit') submit();
      else if (action === 'toggle-backup') toggleBackup(event.target.checked);
      ...
      else if (action === 'choose-playlist') choosePlaylist(actionNode.dataset.awField, actionNode.dataset.awPlaylistId);
      else if (action === 'retry-playlist') retryPlaylist(actionNode.dataset.awPlaylistId);
    });
    root.addEventListener('input', event => {
      const input = event.target.closest('[data-aw-field]');
      if (input) {
        state.draft[input.dataset.awField] = input.type === 'checkbox' ? input.checked : input.value;
        delete state.errors[input.dataset.awField]; state.topError = '';
      }
  ...
  function renderSteps() {
    ...
    $('activationWizardSteps').innerHTML = labels.map((label, index) => {
      ...
      const reached = number <= state.maxReachable;
  ...
  function render() {
    ensureModal();
    const labels = steps();
    state.step = Math.min(Math.max(1, state.step), labels.length);
    ...
    $('activationWizardSubtitle').textContent = state.mode === 'activation'
      ...
      : state.mode === 'renewal'
        ? `${state.device?.deviceCode || ''} · cliente e listas serão preservados`
        : `${state.device?.deviceCode || ''} · sem crédito, plano ou validade`;
    renderSteps();
    $('activationWizardBody').innerHTML = state.result ? renderResult() : renderStep();
    $('activationWizardFooter').innerHTML = renderFooter();
    if (!state.result && (state.mode !== 'renewal' ? (state.mode === 'activation' ? state.step : state.step + 2) : 0) === 3) renderPlaylistResults('primary');
    if (!state.result && (state.mode !== 'renewal' ? (state.mode === 'activation' ? state.step : state.step + 2) : 0) === 4) renderPlaylistResults('backup');
  }
  ...
  function renderFooter() {
    ...
    const labels = steps();
    const spinner = state.busy ? '<span class="aw-spinner" aria-hidden="true"></span>' : '';
  ...
  function renderPlanStep() {
    ...
    const balance = balanceSummary(plan);
    const expiry = plan ? projectedExpiry(plan, d.expiresAt || '', false) : null;
  ...
  function playlistCard(item, field, selected) {
    const info = lifecycleInfo(item);
    const unavailable = playlistUnavailable(item);
    const chosen = String(item.id) === String(selected || '');
    const host = item.host || item.primaryHost || 'Servidor identificado';
    const items = Number(item.cacheItemCount || 0);
    const caps = ['android', 'lg', 'samsung'].map(platform => platformLabel(platform, platformStatus(item, platform)));
    return `<button type="button" class="aw-playlist-card ${chosen ? 'selected' : ''} ${unavailable ? 'unavailable' : ''}" data-aw-action="choose-playlist" data-aw-field="${field}" data-aw-playlist-id="${esc(item.id)}" ${unavailable ? 'disabled' : ''}><span class="aw-playlist-card-head"><strong>${esc(item.name)}</strong><span class="aw-status ${['ready_cache','confirmed_by_device'].includes(info.status) ? 'ok' : unavailable ? 'err' : 'warn'}">${esc(info.label)}</span></span><span class="aw-playlist-meta">${esc(host)} · ${items.toLocaleString('pt-BR')} item(ns)</span><span class="aw-platform-row">${caps.map(text => `<em>${esc(text)}</em>`).join('')}</span></button>`;
  }

  function qualificationNotice(playlist) {
    const info = lifecycleInfo(playlist);
    const tone = ['ready_cache','confirmed_by_device'].includes(info.status) ? 'ok' : playlistUnavailable(playlist) ? 'err' : 'warn';
    const currentPlatform = platformLabel(devicePlatform(), platformStatus(playlist, devicePlatform()));
    const retry = info.status === 'device_failed' || info.status === 'blocked' ? `<button type="button" class="btn" data-aw-action="retry-playlist" data-aw-playlist-id="${esc(playlist.id)}">Tentar processar novamente</button>` : '';
  ...
  function choosePlaylist(field, playlistId) {
    const item = playlistById(playlistId);
    if (!item || playlistUnavailable(item)) return;
    state.draft[field] = playlistId;
    delete state.errors[field]; state.topError = ''; clearAttempt(); render();
  }

  async function retryPlaylist(playlistId) {
    try {
      state.busy = true; render();
      await panelApi(PLAYLIST_FUNCTION, { action: 'retry', playlistId });
      state.busy = false; await refreshData(); render(); watchPlaylist(playlistId);
  ...
  function renderActivationReview() {
    ...
    const plan = planById(d.planId || state.device?.planId);
    const balance = balanceSummary(plan);
    const expiry = projectedExpiry(plan, d.expiresAt || '', false);
    const provisional = [primary, backup].filter(Boolean).some(item => platformStatus(item, devicePlatform()) === 'provisional');
  ...
  function renderRenewalPlan() {
    ...
    const plan = planById(d.planId || state.device?.planId);
    const balance = balanceSummary(plan);
    const expiry = plan ? projectedExpiry(plan, d.expiresAt || '', true) : null;
    const last = recentRenewal(state.device?.id);
  ...
  function renderRenewalReview() {
    ...
    const plan = planById(d.planId || state.device?.planId);
    const balance = balanceSummary(plan);
    const expiry = projectedExpiry(plan, d.expiresAt || '', true);
    const last = recentRenewal(state.device?.id);
  ...
  function validateStep() {
    ...
    if (state.mode === 'renewal') {
      if (state.step === 1) {
        ...
        const recent = recentRenewal(state.device?.id);
        if (recent.recent && !d.confirmRecentRenewal) invalidate('confirmRecentRenewal', 'Confirme a renovação repetida para continuar.');
      ...
      if (logical === 1) {
        ...
        const phone = String(d.customerWhatsapp || '').replace(/\D/g, '');
        if (phone.length < 10 || phone.length > 15) invalidate('customerWhatsapp', 'Informe DDD + número com 10 a 15 dígitos.');
      ...
      if (logical === 3) {
        const item = playlistById(d.playlistId);
        if (!d.playlistId) invalidate('playlistId', 'Escolha a lista principal.');
        else if (!item || playlistUnavailable(item)) invalidate('playlistId', 'Esta lista não está disponível para o tipo deste aparelho.');
      }
      if (logical === 4) {
        if (d.useBackup && !d.backupPlaylistId) invalidate('backupPlaylistId', 'Escolha a reserva ou desative essa opção.');
        const backup = d.backupPlaylistId ? playlistById(d.backupPlaylistId) : null;
        if (d.useBackup && backup && playlistUnavailable(backup)) invalidate('backupPlaylistId', 'Esta reserva não está disponível para este aparelho.');
    ...
    if (Object.keys(state.errors).length) {
      render();
      setTimeout(() => document.querySelector('#activationWizard .aw-field-error')?.closest('label')?.querySelector('input,select,textarea')?.focus(), 0);
  ...
  async function refreshData() {
    const [dashboard, official, universal] = await Promise.all([
      ...
      panelApi(SOURCE_FUNCTION, { action: 'list' }).catch(() => ({ sources: [] })),
    ]);
    const officialById = new Map((official.playlists || []).map(item => [String(item.id), item]));
    const sourceById = new Map((universal.sources || []).map(source => {
      const endpoints = source.endpoints || [];
      const primary = endpoints.find(endpoint => endpoint.primary) || endpoints[0] || {};
      return [String(source.id), { host: primary.host || null, endpointCount: endpoints.length }];
    }));
    dashboard.playlists = (dashboard.playlists || []).map(item => ({ ...item, ...(officialById.get(String(item.id)) || {}), ...(sourceById.get(String(item.id)) || {}) }));
    for (const item of official.playlists || []) {
      if (!dashboard.playlists.some(existing => String(existing.id) === String(item.id))) dashboard.playlists.push({ ...item, ...(sourceById.get(String(item.id)) || {}) });
    }
  ...
  async function openActivation(deviceCode) {
    ...
    await refreshData();
    resetFlow('activation', lookup.device, { customerName: lookup.device.customerName || '', customerWhatsapp: lookup.device.customerWhatsapp || '', customerNotes: '' });
  }
  async function openRenewal(deviceId) {
    await refreshData(); const device = (state.data.devices || []).find(item => item.id === deviceId); if (!device) throw new Error('Aparelho não encontrado.');
    resetFlow('renewal', device, { planId: device.planId || '', expiresAt: '', confirmRecentRenewal: false });
  }
  async function openChange(deviceId) {
    await refreshData(); const device = (state.data.devices || []).find(item => item.id === deviceId); if (!device) throw new Error('Aparelho não encontrado.');
    resetFlow('change', device, { playlistId: device.playlistId || '', backupPlaylistId: device.backupPlaylistId || '', useBackup: Boolean(device.backupPlaylistId) });
  }
  ...
  function close() {
    ...
    stopWatch(); window.RonecaUniversalPlaylists?.close?.(); $('activationWizard')?.classList.remove('open'); state.busy = false; state.attempt = null; state.result = null;
  }

  async function newPlaylist(field) {
    if (!field) return;
    syncInputs();
    const host = $('awInlinePlaylistHost');
    if (!host) return;
    const api = window.RonecaUniversalPlaylists;
    if (!api?.openInline) {
      state.topError = 'O cadastro universal ainda está carregando. Aguarde alguns segundos e tente novamente.'; render(); return;
    }
    await api.openInline(host, {
      onSaved: async result => {
        await refreshData();
        state.draft[field] = result.playlistId;
        clearAttempt();
        setTimeout(() => { render(); watchPlaylist(result.playlistId); }, 0);
      },
      onClose: () => setTimeout(render, 0),
    });
  }
  ...
  async function watchPlaylist(playlistId) {
    ...
    const tick = async () => {
      attempts += 1;
      try {
        const result = await panelApi(PLAYLIST_FUNCTION, { action: 'status', playlistId });
        const current = playlistById(playlistId);
        if (current && result.playlist) Object.assign(current, result.playlist);
        const info = lifecycleInfo(result.playlist || current);
        state.watchText = ['ready_cache','confirmed_by_device'].includes(info.status)
          ? 'Lista pronta. Você pode continuar.'
          : playlistUnavailable(result.playlist || current)
            ? info.message
            : 'Lista salva. O cache continua em segundo plano; no Android você já pode continuar provisoriamente.';
  ...
  function toggleBackup(enabled) { syncInputs(); state.draft.useBackup = Boolean(enabled); if (!enabled) state.draft.backupPlaylistId = ''; clearAttempt(); clearErrors(); render(); }
  function go(step) {
    if (state.busy || state.result) return;
    const target = Math.max(1, Math.min(Number(step || 1), steps().length));
    if (target > state.maxReachable) return;
  ...
  function next() {
    if (state.busy) return;
    try { validateStep(); state.maxReachable = Math.max(state.maxReachable, Math.min(steps().length, state.step + 1)); state.step += 1; render(); setTimeout(() => $('activationWizardBody')?.querySelector('input,select,textarea,button')?.focus(), 0); } catch { /* erros já visíveis */ }
  }
  ...
  async function submit() {
    try {
      ...
      if (state.mode === 'activation') {
        action = 'activate'; payload = { deviceId: state.device.id, customerName: d.customerName.trim(), customerWhatsapp: d.customerWhatsapp.trim(), customerNotes: String(d.customerNotes || '').trim() || null, planId: d.planId, playlistId: d.playlistId, backupPlaylistId: backup, expiresAt: d.expiresAt ? customExpiryIso(d.expiresAt) : null };
      } else if (state.mode === 'renewal') {
        action = 'renew'; payload = { deviceId: state.device.id, planId: d.planId, expiresAt: d.expiresAt ? customExpiryIso(d.expiresAt) : null };
      } else {
        action = 'changePlaylists'; payload = { deviceId: state.device.id, playlistId: d.playlistId, backupPlaylistId: backup, reason: 'Alteração pelo fluxo comercial do vendedor' };
      }
      ...
      await window.loadPortal?.(); await window.RonecaSellerPortal?.refresh?.();
      setTimeout(close, 900);
    } catch (error) {
      state.busy = false; state.result = null; state.topError = error.message || 'Não foi possível concluir. Seus dados foram preservados; corrija o problema e tente novamente.'; render();
    }
  }
