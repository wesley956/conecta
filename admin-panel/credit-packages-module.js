(() => {
  ...
  function adminHtml(){
    return `<div class="credit-packages-shell">
      <div class="credit-packages-head"><div><small>Financeiro da empresa</small><h2>Venda de créditos</h2><p>Somente as compras feitas pelos vendedores aparecem aqui. O financeiro particular deles permanece privado.</p></div><button class="btn primary" onclick="creditPackagesOpenSale()">Nova venda</button></div>
      ...
      <div class="credit-package-card"><div class="credit-package-card-head"><div><h3>Compras dos vendedores</h3><p>Histórico de pacotes, pagamentos e validade.</p></div></div><div class="credit-orders-wrap"><table><thead><tr><th>Data</th><th>Vendedor</th><th>Pacote</th><th>Créditos</th><th>Pagamento</th><th>Liberação</th><th>Validade</th><th>Valor</th><th>Ação</th></tr></thead><tbody id="cpOrders"></tbody></table></div></div>
      <div class="credit-package-card"><div class="credit-package-card-head"><div><h3>Limite por vendedor</h3><p>Autorize compra a prazo e defina o limite máximo de dívida.</p></div></div><div id="cpSellerTerms" class="credit-seller-terms"></div></div>
    </div>`;
  }

  function sellerHtml(){
    return `<div class="credit-packages-shell"><div class="credit-packages-head"><div><small>Meus créditos</small><h2>Compras e validade</h2><p>Seu financeiro de clientes continua privado e separado destas compras.</p></div></div>
  ...
  function ensureSeller(){
    ...
    if(!nav.querySelector('[data-seller-nav="credit-purchases"]')){ const b=document.createElement('button'); b.dataset.sellerNav='credit-purchases'; b.type='button'; b.textContent='Meus créditos'; b.onclick=()=>{window.sellerPortalNavigate?.('credit-purchases');load();}; nav.appendChild(b); }
  ...
  function renderPackages(target){
    $(target).innerHTML=(data.packages||[]).map(pkg=>`<article class="credit-package-option ${pkg.code==='BASICO_50'?'featured':''}"><small>${esc(pkg.name)}</small><strong>${pkg.credits} créditos</strong><b>${money(pkg.price_cents)}</b><span>Validade de ${pkg.validity_days} dias</span>${pkg.code==='BASICO_50'?'<em>Mais vantajoso</em>':''}</article>`).join('');
  }
  ...
  function renderAdmin(){
    ...
    $('cpOrders').innerHTML=(data.orders||[]).length?(data.orders||[]).map(o=>`<tr><td>${date(o.createdAt)}</td><td>${esc(o.sellerName||'—')}</td><td>${esc(o.packageName)} × ${o.packageQuantity}</td><td>${o.creditsTotal}</td><td><span class="cp-status ${o.paymentStatus}">${esc(o.paymentStatus)}</span></td><td>${esc(o.creditsStatus)}</td><td>${date(o.expiresAt)}</td><td>${money(o.totalAmountCents)}</td><td>${o.paymentStatus!=='paid'&&o.paymentStatus!=='cancelled'?`<button onclick="creditPackagesMarkPaid('${o.id}')">Marcar pago</button>`:''}</td></tr>`).join(''):'<tr><td colspan="9">Nenhuma compra registrada.</td></tr>';
    $('cpSellerTerms').innerHTML=(data.sellers||[]).map(s=>`<div class="credit-seller-term"><div><strong>${esc(s.name)}</strong><span>Saldo: ${s.creditBalance} créditos</span></div><label><input id="cpTermsAllowed-${s.id}" type="checkbox" ${s.allowCreditPurchasesOnTerms?'checked':''}> Compra a prazo</label><label>Limite (R$)<input id="cpTermsLimit-${s.id}" type="number" min="0" step="0.01" value="${(s.financialCreditLimitCents/100).toFixed(2)}"></label><button onclick="creditPackagesSaveTerms('${s.id}')">Salvar</button></div>`).join('');
  }
  ...
  function renderSeller(){
    ...
    $('cpSellerOrders').innerHTML=(data.orders||[]).length?(data.orders||[]).map(o=>`<article><div><strong>${esc(o.packageName)} × ${o.packageQuantity}</strong><span>${o.creditsTotal} créditos · ${date(o.createdAt)}</span><small>Pagamento: ${esc(o.paymentStatus)} · Créditos: ${esc(o.creditsStatus)} · Validade: ${date(o.expiresAt)}</small></div><b>${money(o.totalAmountCents)}</b></article>`).join(''):'<p>Nenhuma compra registrada.</p>';
  }
  ...
  window.creditPackagesOpenSale=function(){
    ...
    const modal=document.createElement('div'); modal.className='cp-modal open'; modal.id='cpModal'; modal.innerHTML=`<div class="cp-modal-card"><h2>Nova venda de créditos</h2><label>Vendedor<select id="cpSaleSeller"><option value="">Selecione</option>${sellers.map(s=>`<option value="${s.id}">${esc(s.name)}</option>`).join('')}</select></label><label>Pacote<select id="cpSalePackage">${packages.map(p=>`<option value="${p.id}">${esc(p.name)} — ${p.credits} créditos — ${money(p.price_cents)}</option>`).join('')}</select></label><label>Quantidade de pacotes<input id="cpSaleQty" type="number" min="1" value="1"></label><label>Status<select id="cpSaleStatus"><option value="paid">Pago</option><option value="pending">Pendente</option></select></label><label>Pagamento<select id="cpSalePayment"><option value="pix">Pix</option><option value="cash">Dinheiro</option><option value="card">Cartão</option><option value="bank_transfer">Transferência</option><option value="boleto">Boleto</option><option value="other">Outro</option></select></label><label>Liberação<select id="cpSaleRelease"><option value="immediate">Liberar agora</option><option value="after_payment">Liberar após pagamento</option></select></label><label>Vencimento<input id="cpSaleDue" type="date"></label><label>Observação<textarea id="cpSaleNotes"></textarea></label><div class="actions"><button class="btn primary" onclick="creditPackagesSubmitSale()">Confirmar venda</button><button class="btn" onclick="document.getElementById('cpModal').remove()">Cancelar</button></div></div>`; document.body.appendChild(modal);
  };

  window.creditPackagesSubmitSale=async function(){ try{ await api({action:'createOrder',sellerId:$('cpSaleSeller').value,packageId:$('cpSalePackage').value,packageQuantity:Number($('cpSaleQty').value||1),paymentStatus:$('cpSaleStatus').value,paymentMethod:$('cpSalePayment').value,releasePolicy:$('cpSaleRelease').value,dueDate:$('cpSaleDue').value||null,notes:$('cpSaleNotes').value||null,idempotencyKey:operationKey('credit-package')}); $('cpModal')?.remove(); await load(); alert('Venda registrada com sucesso.'); }catch(e){alert(e.message);} };
  window.creditPackagesMarkPaid=async id=>{ try{await api({action:'updatePayment',orderId:id,paymentStatus:'paid'});await load();}catch(e){alert(e.message);} };
  window.creditPackagesSaveTerms=async id=>{ try{const value=Number($(`cpTermsLimit-${id}`).value.replace(',','.'));await api({action:'updateSellerTerms',sellerId:id,allowCreditPurchasesOnTerms:$(`cpTermsAllowed-${id}`).checked,financialCreditLimitCents:Math.round(value*100)});await load();}catch(e){alert(e.message);} };
